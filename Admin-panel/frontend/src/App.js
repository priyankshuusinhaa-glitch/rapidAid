...
import BookingsFare from './components/BookingsFare';
...
function App() {
  ...
  const checkAuth = async () => {
    try {
      ...
      setUser(response.data.user || response.data); // backward compatible
    } catch (error) {
...
{/* 
            ...
            <Route 
              path="/otp" 
              element={user ? <OTPManagement user={user} /> : <Navigate to="/login" />} 
            />
            <Route 
              path="/fares" 
              element={user ? <BookingsFare user={user} /> : <Navigate to="/login" />} 
            />
            <Route 
              path="/" 
